...
def getHeatMap():
    ...
    cont_headers, files = conn.get_container('notebooks') # Object Data Store has the concept of Containers. Containers are a way of dividing files into groups in Object Data Store.
    
    final_json = {}
    finalJSON = {}
    for files in conn.get_container('notebooks')[1]:
        
        if 'part' in files['name']:
            year = re.search("[0-9]{4}",files['name']).group(0)
            print year
            obj = conn.get_object('notebooks',files['name'])[1]
            ...
            obj_new = json.loads(obj_new)
            for district in obj_new:
                if district['PdDistrict'] in final_json:
                    district_json = final_json[district['PdDistrict']]
                    district_json['count'] = district_json['count'] + district['count']
                    final_json[district['PdDistrict']] = district_json
                else:
                    ...
                    final_json[district['PdDistrict']] = district
    finalJSON['data'] = sorted(final_json.values(),key=itemgetter('count'), reverse=True)
    finalJSON['year'] = year
